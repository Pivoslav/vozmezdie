...
def _align_by_content(
    ...
    """
    Align by matching entry_eng or entry_rus text. Returns (aligned_rows, n_human, n_llm, n_matched).
    When match_rus is True, match by entry_rus (Russian); otherwise entry_eng.
    """
    n_human = len(gt_rows)
    n_llm = len(llm_rows)
    pairs = _content_match_pairs_rus(gt_rows, llm_rows) if match_rus else _content_match_pairs(gt_rows, llm_rows)
    aligned = []
    for gt_i, llm_i in pairs:
        llm = llm_rows[llm_i]
        gt = gt_rows[gt_i]
        cat_match = (llm.get("content_category") or "").strip() == (gt.get("content_category") or "").strip()
        fram_match = (llm.get("framing") or "").strip() == (gt.get("framing") or "").strip()
        # Use GT for both entry_eng and entry_rus to ensure we always display a consistent
        # translation pair. Content-based matching can pair rows where one phrase contains
        # the other, which would otherwise mix eng from one segment with rus from another.
        aligned.append({
    ...
    return aligned, n_human, n_llm, len(aligned)
...
def run(
    ...
    for doc_id in llm_by_doc:
        ...
        if n_matched == 0:
            ...
            cat_pct = 100.0 * sum(1 for r in aligned if r["category_match"]) / n_matched
            fram_pct = 100.0 * sum(1 for r in aligned if r["framing_match"]) / n_matched
            both_pct = 100.0 * sum(1 for r in aligned if r["both_match"]) / n_matched
